...
function deObfuscate(input,output) {
    const argv = parseCommandLineArgs();
    const logger = createLogger(argv.debug);
    logger.info(`[index.js] Starting deobfuscation process for file ${input}.`);

    const obfuscatedSourceCode = readTextFile(input);
    ...
    usefulModule.writeTextFile(output, deobfuscatedSourceCode);
